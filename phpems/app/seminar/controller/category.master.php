	...
	private function add()
	{
		if($this->ev->get('addcategory'))
		{
			...
			$message = array(
			    ...
			    "forwardUrl" => "index.php?seminar-master-category&parent={$parent}&page={$page}"
			);
		...
		else
		{
			...
			$tpls = array();
			foreach(glob("app/seminar/tpls/app/category_*.tpl") as $p)
			{
	...
	private function edit()
	{
		...
		if($this->ev->get('submit'))
		{
			...
			$message = array(
				...
				"message" => "操作成功",
			    "callbackType" => "forward",
			    "forwardUrl" => "index.php?seminar-master-category&parent={$cat['catparent']}&page={$page}"
			);
			exit(json_encode($message));
		}
		else
