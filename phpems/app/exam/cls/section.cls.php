...
class section_exam
{
    ...
    public function getQuestionsByKnows($knowsid)
    {
		...
		{
			$data = array('DISTINCT questionid,questiontype',array('questions','quest2knows'),array(array("AND","find_in_set(quest2knows.qkknowsid,:knowsid)",'knowsid',$knowsid),array("AND","quest2knows.qktype = 0"),array("AND","quest2knows.qkquestionid = questions.questionid"),array("AND","questions.questionstatus = 1")),false,"questionparent asc,questionsequence asc,questionid asc",false);
			$sql = $this->pdosql->makeSelect($data);
            ...
            $data = array('DISTINCT questionid,qrtype',array('questions','questionrows','quest2knows'),array(array("AND","find_in_set(quest2knows.qkknowsid,:knowsid)",'knowsid',$knowsid),array("AND","quest2knows.qktype = 1"),array("AND","quest2knows.qkquestionid = questionrows.qrid"),array("AND","questions.questionparent = questionrows.qrid"),array("AND","questionrows.qrstatus = 1")),false,"questionparent asc,questionsequence asc,questionid asc",false);
            $sql = $this->pdosql->makeSelect($data);
            $r = $this->db->fetchAll($sql);
            foreach($r as $p)
            {
